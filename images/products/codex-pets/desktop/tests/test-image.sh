...
project="products/codex-pets/desktop"
...
grep -Fq 'kasm-patch "Codex Pets"' "$dockerfile"
...
if [ -d "$overlay/etc/desktop/startup.d" ]; then
    echo "Codex Pets installs an X-dependent program under startup.d." >&2
    exit 1
fi
...
echo "Codex Pets image checks passed."
